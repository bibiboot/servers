...
-------------------------------------------
Use TCP_NODELAY. As the nache algorithm applies certain rules to see if the data is to be written to the system or not. i.e. to be flushed out of the buffer.
...
What is TCP_NODELAY?
-------------------------------------------
It prevents buffering by stopping the use of Nagles algorithm thereby increasing network traffic.
...
What is RST
-------------------------------------------
RST is send by the agent which is doign the active close.
RST is used to denote connection reset in case of emergency.
SO_LINGER sets a timeout on a socket and then closes the connection by sending RST.

We can send data instead of string but as struct but the problem will be , when sending the negative integer inside the structure long and int format will become and issue. Therefore always send data as string.
